...
export default async function ClientesPage({
  ...
  const [{ data: perfil }, { data: clientes, count }, { data: tendencias }, { data: fees }] = await Promise.all([
    ...
    supabase.from("clientes").select("id, fee_porcentaje, fee_interior_porcentaje"),
  ]);
  ...
  const feesPorCliente = new Map<string, { camara: number; interior: number | null }>();
  for (const cl of fees ?? []) {
    feesPorCliente.set(cl.id, {
      camara: Number(cl.fee_porcentaje),
      interior: cl.fee_interior_porcentaje != null ? Number(cl.fee_interior_porcentaje) : null,
    });
  }
          ...
          <table className="w-full text-sm">
            <thead className="bg-card/80 text-left text-[11px] uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium">Fee Cámara</th>
                <th className="px-4 py-3 text-right font-medium">Fee Interior</th>
            ...
            <tbody className="divide-y divide-border bg-background">
              {(clientes ?? []).map((c) => (
                <tr key={c.cliente_id} className="transition hover:bg-muted/40">
                  ...
                  <td className="px-4 py-3 text-right font-mono text-foreground/90">
                    {(feesPorCliente.get(c.cliente_id)?.camara ?? Number(c.fee_porcentaje)).toFixed(2)}%
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-foreground/90" title="Si no tiene fee interior propio, aplica el de camara">
                    {feesPorCliente.get(c.cliente_id)?.interior != null
                      ? (feesPorCliente.get(c.cliente_id)!.interior as number).toFixed(2) + "%"
                      : (feesPorCliente.get(c.cliente_id)?.camara ?? Number(c.fee_porcentaje)).toFixed(2) + "% (=cam.)"}
                  </td>
                  <td className="px-4 py-3">
              ...
              {(clientes ?? []).length === 0 && (
                <tr>
                  <td colSpan={esAdmin ? 8 : 7} className="px-4 py-10 text-center text-muted-foreground">
